...
def path_traversal_hillshade(world, full_path, waypoints):
    ...
    travel_distance = len(full_path)*10 # ten meters per pixel
    print(f'Distance traveled: {travel_distance} m')

    ax.set_title(f"Lunar Rover Traverse (D*) - {travel_distance}m Path")
    # ax.axis('off')
...
def store_path_results(full_path, waypoints):
    csv_file_path = "planned_path_nodes.csv"
    ...
    with open(csv_file_path, mode='w', newline='') as file:
        ...
        writer.writerow(["x", "y", "LOS?"])
        for wp in range(len(waypoints)):
            writer.writerow([waypoints[wp]])
...
# sm = matplotlib.cm.ScalarMappable(cmap='viridis', norm=matplotlib.colors.Normalize(vmin=np.min(slopes), vmax=np.max(slopes)))
# sm.set_array([])
