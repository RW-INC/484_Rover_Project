...
curr_state = [
    ...
    0;                      % 7. Roll
    0;                      % 8. Pitch
    my_test.traj.Theta(1)   % 9. Yaw (Starting heading)
];
curr_state(end) = mod(curr_state(end), 2*pi);
scale = 0.8;
sun_scale = 12;
...
while ~boundary_hit
    for sub = 1:steps_per_frame
        ...
        % =============================================
        dstate = FullStateDynamics(curr_state, u, my_test, geom, dt,ctrl_freq);
        curr_state = curr_state + dt * dstate;
        curr_state(7) = mod(curr_state(7), 2*pi);
        yaw_true   = curr_state(9); 
        pitch_true = curr_state(8); 
        roll_true  = curr_state(7);
        ...
        % Convert global Euler rates to local Body rates (p, q, r)
        p_true = droll_t - sin(pitch_true) * dyaw_t;
        q_true = cos(roll_true) * dpitch_t + sin(roll_true) * cos(pitch_true) * dyaw_t;
        r_true = -sin(roll_true) * dpitch_t + cos(roll_true) * cos(pitch_true) * dyaw_t;
        w_body_true = [p_true; q_true; r_true];
        
        % Add noise to the body rates (which is what a real IMU measures)
        w_imu = w_body_true + bias_rate_gyro * dt + randn(3,1) * sigma_gyro;
        ...
        if mod(i-1, sim_freq/estimation_freq) == 0
            ...
            [mekf_state, mekf_P] = rotational_mekf_fixed(...
                q_triad_obj, w_imu_mod, mekf_state, mekf_P, u, dt * sim_freq / estimation_freq, ...
                sigma_gyro);
            ...
            [trans_state, trans_P] = translational_ekf_fixed(...
                ...
                [roll_est;pitch_est;yaw_est], u, trans_state, trans_P, mekf_P, dt * sim_freq / estimation_freq);
